...
class TumblrDatabase
  ...
  def create_tables
    ...
    # create the tumblr_audio table
    @db.execute <<-SQL
      CREATE TABLE IF NOT EXISTS tumblr_audio (
        post_id INTEGER,
        audio_id INTEGER,
        member_name TEXT,
        save_name TEXT,
  ...
  def insert_tumblr_audio(post_id, audio_id, member_name, save_name)
    created_date = DateTime.now.strftime('%Y-%m-%d %H:%M:%S')
    @db.execute("INSERT INTO tumblr_audio (post_id, audio_id, member_name, save_name, created_date) VALUES (?, ?, ?, ?, ?)", post_id, audio_id, member_name, save_name, created_date)
  end
